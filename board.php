      ...
      <a href="login/login_name.php">로그인</a>  <a href="index.php">Home</a>
        ...
        <nav id="top_menu2">
          <ul>
            ...
            <div class="nor" onclick="location.href='board.php?bid=board';">  <li>질문게시판</li></div>
          </ul>
        </nav>
      </header>
	 <body>
...
<div id="side">
	  <?php
	$bid = empty($_REQUEST["bid"]) ? "board" : $_REQUEST["bid"];
	$bid='board';
	session_start();
...
<div class="str">    
 <td style="text-align:left;"> 
   <div class=<?=$side_bord?> onclick="location.href='board.php?bid=<?=$str?>&pages=<?=$pages?>&mainbid=board'"><?php echo"$str"?></div>
 </td>
...
?>
        ...
        <div id="">
          <img id ="img"src="img/board.png" alt="">
        </div>
...
<?php		
		if($firstLink>1){
?>
			<a class="nones" href="board.php?bid=<?=$bid?>&page=<?= $firstLink -1 ?>"> 이전 </a>
<?php
	 }
		
		for ($i=$firstLink; $i <= $lastLink; $i++){		
?>
			<a class="nones" href="board.php?bid=<?=$bid?>&page=<?=$i?>"> <?=($i == $page) ? "<u>$i</u>" : $i?> </a>
<?php
		}
		
		if($lastLink<$numPage){
?>
			<a class="nones" style= 'margin:0;' href="board.php?bid=<?=$bid?>&page=<?=$lastLink +1?>"> 다음 </a>
<?php
		}
...
<br><br>
<?php
if($_SESSION["userId"]!="admin" and $bid=='공지사항'){ 
?>
