...
<div id="all">
    ...
    <form action="board/search_result.php" method="get">
      ...
      <a href="login/login_name.php">로그인</a>  <a href="index.php">Home</a>
        ...
        <nav id="top_menu2">
          ...
          <div class="nor" onclick="location.href='yesno.php?bid=yesno';">  <li>찬반주제</li></div>
 ...
 <div id="side">
	  <?php
	$bid = empty($_REQUEST["bid"]) ? "yesno" : $_REQUEST["bid"];
	$bid='yesno';
	session_start();
...
<div class="str">    
 <td style="text-align:left;"> 
   <div class=<?=$side_bord?> onclick="location.href='yesno.php?bid=<?=$str?>&pages=<?=$pages?>&mainbid=yesno'"><?php echo"$str"?></div>
 </td>
...
?>
        ...
        <div id="">
          <img id ="img"src="img/yesno.png" alt="">
        </div>
...
while ($row = $query->fetch()) {
    ...
    $mainbid = empty($_REQUEST["mainbid"]) ? "yesno" : $_REQUEST["mainbid"];
...
?> 
     ...
     odata[<?= $jsnum ?>] =  ['',  <?= $yes ?>,   <?= $mid ?>, <?= $no ?>, ''];      
                ...
                <tr class="borard_link">
                    ...
                    <td style="text-align:left;">
                     <a href="board/view.php?bid=<?= $bid ?>&num=<?php echo "$row[num]";?>&page=<?= $page ?>&mainbid=<?= $mainbid ?>">
					 <?php echo "$row[title]";?><?php echo "[", $data, "]";?><?php echo $img ?></a></td>
					<td><div id="jsc_<?= $jsnum ?>" style="width: 200PX; height: 150PX;"></div>    </td>
...
if ($firstLink > 1) {
?>
           <a class="nones" href="yesno.php?bid=<?= $bid ?>&page=<?= $firstLink - $numLinke ?>&jsnum=<?= $jsnum ?>"> 이전 </a>
<?php
}

for ($i = $firstLink; $i <= $lastLink; $i++) {
?>
           <a class="nones" href="yesno.php?bid=<?= $bid ?>&page=<?= $i ?>&jsnum=<?= $jsnum ?>"> <?= ($i == $page) ? "<u>$i</u>" : $i ?> </a>
<?php
}

if ($lastLink < $numPage) {
?>
           <a class="nones" style= 'margin:0;'  href="yesno.php?bid=<?= $bid ?>&page=<?= $firstLink + $numLinke ?>&jsnum=<?= $jsnum ?>"> 다음 </a>
<?php
}
...
<input type="button" value="글쓰기"
       onclick="location.href='board/write.php?bid=<?= $bid ?>&a=<?= $a ?>'">
...
function drawBasic() {
       ...
       var options_fullStacked = {
        ...
        for (i = 0; i < <?= $jsnum ?>; i++){
            ...
            ['', '찬성', '중간', '반대', { role: 'annotation' } ],
            odata[i],
    
            ]);
      
            
            var chart = new google.visualization.BarChart(document.getElementById('jsc_'+i));
